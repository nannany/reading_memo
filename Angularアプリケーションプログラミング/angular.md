...
ユニットテストを実施するために必要なものとしては、
* テスティングフレームワーク：スクリプトで表現されたテストを実施し、テスト対象コードが期待された値を返すか検証するためのツール。angular標準は、Jasmine
* テストランナー：テストを実行するためのツール。Karmaが代表的。

### 10.2.2 ユニットテストの準備
karma.conf.js の説明が載っている。

### 10.2.3 テストの基本
describe(name, spec)メソッドでテストを記述する。nameはテストスイートの名前でspecにテストケース群を記述する
個々のテストケースはit(name,　test)メソッドで記述する。nameはテストケースの名前で、testがテストの内容
テスト検証ではexpect(resultValue).matcher(expectValue)メソッドを用いる。resultValueがテスト対象コード、matcherが検証メソッド、expectValueが期待する値。
 
テストの実施に際しては、npm testコマンドを実行する。

## 10.3 ユニットテスト（Angularアプリ)

### 10.3.1 パイプのテスト
パイプクラスをインスタンス化し、そこでメソッドを使用してmatcherにかける。

### 10.3.2 サービスのテスト

### 10.3.3 コンポーネントのテスト（基本）
コンポーネントのテストにおいては、angularが内部的に行ってきた処理をテストコードとして明示的に書く必要がある。

* TestBed.configureTestingModule(def)メソッドでテストモジュールを準備する。defではモジュールの定義情報を入れる。
* TestBed.createComponent(comp)メソッドでテスト対象のメソッドをインスタンス化する。

上記をbeforeEachメソッド内に書く必要がある。

fixture.detectChanges()メソッドが実行されたときに、テンプレートにコンポーネントの値が反映される。
 
AngularのテストAPIを利用しないテストのことを、Isolated Unit Tests（分離単体テスト）という。

以下の設定を入れることによって、変更検知の自動化ができる。ただし、コンポーネントへの直接の変更は検知できない。
...
テンプレートが外部化されている場合、TestBed#compileComponentメソッドでテストモジュール配下のすべてのコンポーネントを非同期で読み込み、コンパイルする。

### 10.3.5 入出力を伴うコンポーネントのテスト
@Inputの部分のテストは、@Inputで修飾されたプロパティに値を代入して、detectChangesメソッドを実行すればよい。
@Outputの部分を補足してテストするには、triggerEventHandler(name, ev)メソッドでイベントを発生させて、行う。

### 10.3.6 サービスに依存したコンポーネントのテスト
依存しているサービスをそのまま注入するのではなく、テストダブル（stubみたいなもの）を使用する。
overrideComponent(component, override) メソッドを用いることで、コンポーネントの設定を上書くことができる。

### 10.3.7 サービスに依存したコンポーネントのテスト（スパイ）
spyの生成においては、spyOn(obj, method)メソッドを使う。objが割込み対象のメソッドで、methodが割り込むメソッド。

### 10.3.8 サービスに依存したコンポーネントのテスト（非同期版）

### 10.3.9 「浅い」コンポーネントのテスト-NO_ERRORS_SCHEMA
NO_ERRORS_SCHEMAオプションを利用することによって、認識できない要素/属性を無視することができる。

### 10.3.10 ディレクティブのテスト-ホストコンポーネント
ディレクティブのテストは、テストのためのホストコンポーネントを用意して、そのコンポーネント経由で行うべき。

### 10.3.11 Http/Jsonpを利用したサービスのテスト
Http/Jsonpサービスがダミーのデータを返すようにモックを用意する。
ReflectiveInjector.resolveAndCreate(providers[, parent]) メソッドで、モックを生成できる。
providersにはProvider宣言、parentには親となるインジェクターを入れる。

#### テストダブルの種類

* ダミー：メソッド呼び出しの数合わせに利用するオブジェクト（操作は行われないことが前提）
* フェイク：実際のオブジェクトをより簡素に模倣したオブジェクト（例：インメモリデータベース）
* スタブ：あらかじめ用意した結果を返すオブジェクト。その結果をもとにテスト対象を動作/検証する。
* モック：テスト対象から渡された引数を検証するオブジェクト。スタブと異なり、モックを正しく呼び出しているかを検証する。
* スパイ：テスト対象コードで、メソッドを呼び出したかどうか。その回数/引数などを記録/検証するためのオブジェクト（スタブのようにあらかじめ結果を設定しておくことも可）

## 10.4 E2E（End to End）テスト
インテグレーションテストのこと。

### 10.4.1 E2Eテストの準備
Protractorを使用するのが一般的。内部的にはSelenium WebDriverを使用している。
準備として、Selenium Serverをインストールする必要がある。以下のコマンドをたたく。
`npm run preprotractor`

### 10.4.2 E2Eテストの基本
browserオブジェクトでブラウザを操作できる。
elementメソッドで目的の要素を取得できる。
`npm run e2e`でテスト実行。
サンプルそのままやると失敗する？？
...
### 11.1.1 BootstrapをAngularアプリで活用する-ngx-bootstrap
BootstrapはCSSのフレームワーク。
有効にするには、

* package.jsonに依存関係を記述。
* メインページからBootstrapを読み込む。
* systemjs.config.jsに追記。
* ルートモジュールに登録。


### 11.1.2 標準の検証ルールを拡張する-ng2-validation
Angular標準以上の検証ルールを提供してくれる。
有効にするには、
...
コンポーネント、サービスのスケルトンを用意してくれるツールがAngular CLI。
ng generate コマンドでコンポーネント/サービスの自動生成を行える。
ng build コマンドでアプリで必要なファイル一式を準備することができる。
ng get/set コマンドでAngular CLIの設定を取得、更新できる。

### 11.2.2 Angularアプリの構造を視覚化する-Augury
chromeの拡張機能

### 11.2.3 アプリを事前コンパイルする-AoTコンパイル
事前にコンパイルしてサイズを減らしたりする。
