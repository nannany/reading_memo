...
ソフトウェアアーキテクトが手を動かすことをやめるべきではない。

プロジェクトポートフォリオを作成する。
ポートフォリオを作るために、次の問いに答える。
...
アーキテクチャを図やドキュメントやプロトタイプとして形にする
...
#### 5.2.2 具体的かつ測定可能な応答測定とするように努める

#### 5.2.3 取り組もう：メモを洗練させて品質特性シナリオにする
...
ASRを見つけるための手法
* GQMワークショップ(アクティビティ3)
* ステークホルダーインタビュー(アクティビティ4)
* 前提リスト(アクティビティ5)
* ミニ品質特性ワークショップ(アクティビティ7)
* ミニ品質特性ワークショップ(アクティビティ7)
...
次には、今わかっていることに基づいて設計の選択肢を検討し、判断を下す方法を学んでいく

## 6章 アーキテクチャを選ぶ(君がアーキテクチャに選ばれる前に)

ASRに基づいた意思決定を推進することで、アーキテクチャの構造を選択する方法を学んでいく

### 6.1 選択肢を広げるために発散させ、決定するために収束させる

すべての選択肢を俎上にあげることはできない

#### 6.1.1 アーキテクチャ上重要なものを探求する

* アーキテクチャ構造の全体構成を決定するために、要素とその役割を探求する
* 要素がどう相互作用するかを決定するために、関係とそのインターフェースを探求する
* アーキテクチャが形どる世界を理解するためにドメインを探求する
...
与えられた制約と自分自身で与えた制約を常に区別する

### 6.3 望ましい品質特性を促進する

すべての設計は再設計。望ましい品質特性を促進するパターンを見つけ、そのパターンをアーキテクチャの出発点として使う
