...
AWSのアベイラビリティーゾーンなどは、バルクヘッドの仕組みを利用している。
 
以下の図のような構成をしているシステムにおいて、Fooで故障が起こったとする。  
その故障が引き金となって、Bazも動かなくなったら、Barも止まってしまう。  
このようなことが起こった場合、Bar側からは障害解析が難しい。また、Foo、Bar合わせてメンテの時間を取る、Foo、Barのクライアント窓口はどうするか等の問題も残る。
 
![no_bulkhead](リリースイット_4章_11.bmp)

厳しいSLAが定められている場合、以下のような構成にして、依存先を分離するべき。  
デッドロック等の問題は残るだろうが、それはまた別のアンチパターンである。

![bulkhead](リリースイット_4章_12.bmp)


### 覚えておくこと

* 船の一部を守る
  * バルクヘッドは、何か悪いことが起こったときに、それ以外の機能を守るために、キャパシティ（？）を分離しておく設計パターンである
* 意味のある粒度を選択する
  * アプリケーション内のスレッドプール、サーバ内のCPU、クラスター内のサーバ等で区分けできる
* 特に共有サービスモデルではバルクヘッドパターンを考慮する
  * SOAやマイクロサービスアーキテクチャでの故障は、とても早く広がりうる。サービスが連鎖反応で落ちていく時に、会社全体が止まってしまうのを防ぐために、バルクヘッドを用意しておくのが良い
...
* fiddlingを避ける
  * 人手の介入は問題を招くので、なくす必要がある。マニュアルでのディスククリーンアップや毎晩の再起動なしに、典型的なデプロイサイクルを回せるようにしておくべき
* アプリケーションロジックでデータを消す
  * DBAはデータを削除するスクリプトを作れるが、データを消した場合に、アプリケーションがどのように動作するかを必ずしも知ってはいない。論理的整合性を保つため、特にORMを使っているようなときには、アプリケーションで自身のデータを消せるようにしておく
* キャッシュを制限する
...
外部エンドポイントを叩くモジュールにて、応答がなく、タイムアウトの設定もなかったため、コネクションプールの枯渇が起きていた模様。

DRPというプロトコル、一般的な名称なのか？？当然のように語られているが。
この内容のどのあたりがDiagnostic tests なのか、正直読み取れなかった。

### Call In a Specialist

奥の方のサーバーが弱い状況だった。

![](スクリーンショット%202021-10-10%2017.59.06.png)

インシデントが起きている中で、対応策を考えなければいけない過酷な状況に陥った。

### Compare Treatment Options

呼び出しのスロットリングが必要ということがわかってきた。

通常のリソースプールと、order managementへの通信に用いるコネクションプールが異なるプールで管理されていたので、それを利用して流量制御することにした。

### Does the Condition Respond to Treatment?

コネクション系の設定をうまくいじったらうまくいったらしい。正直よくわからん。。
