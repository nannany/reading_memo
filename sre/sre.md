...
* Googleデータセンターの構成を言葉で表すと以下のよう
  * 十数台のマシンが1つのラックに配置されている
  * 複数のラックが1列に並べられている
...
* ハードウェアの故障は頻繁に起こるが、それをユーザーに意識させないようにしている。
...
* Chubbyというロックサービスで、ロックを管理するためのファイルシステムに似たAPIを提供

### 2.3.2 モニタリングとアラート

* Borgmonというモニタリングプログラムを使用して対象サーバのメトリクスを得ている
...
* アラート発生時に人間にページを送って割り込むべき問題と、ページを送るほどでない問題への対処法のガイドラインを示す

## 6.1 定義

* 以下、用語の定義
  * **モニタリング**：システムに関するリアルタイム定量データの収集、処理、集計、表示を行うこと。クエリの回数と種類、エラーの回数と種類、処理時間、サーバーの生存時間等
  * **ホワイトボックスモニタリング**：システムの内部によって公開されているメトリクスに基づくモニタリング。ログ、JVM Profiling InterfaceのようなIF、内部的な統計情報を出力するHTTPハンドラ等
  * **ブラックボックスモニタリング**：ユーザーが目にする外部のふるまいをテストする
  * **ダッシュボード**：サービスの主要メトリクスのサマリビューを提供するアプリケーション
  * **アラート**：人間に読まれることを意図した通知。チケット、メールアラート、ページに分類される
  * **根本原因**：ソフトウェアの欠陥やヒューマンエラーで、修正されれば同じことは起きないと確信できるもの
  * **ノードとマシン**：物理サーバー、仮想マシン、コンテナ内で動作しているカーネルの1つのインスタンスを示す言葉で、どちらも同じ意味で使われる
  * **プッシュ**：サービス動作中のソフトウェア、あるいはその設定に対する変更
...
* 理由は以下のよう
  ...
  * **アラート**
  * **ダッシュボードの構築**
  * **アドホックな振り返り分析の進行（デバッキングなど）**：レイテンシが急上昇したが、他に何か同時に起きたことはないか？
* 単に「何か少しおかしい」程度ではアラートを起こしてはならない。人の時間を奪ってしまうし、多すぎると本物のアラートを見過ごすことにつながる

## 6.3 モニタリングにおける妥当な期待値の設定

* モニタリングシステム、中でもプロダクション環境での問題の発端から人間へのページ、そして基本的な切り分けと深いデバッギングに至るクリティカルパスを、チーム全員でシンプルかつ包括的なものに保つ
* 人間へのアラートを発生させるルールは、理解しやすく、明確な障害を表現するものであるべき

## 6.4 症状と原因

* モニタリングシステムは、何が壊れたのか（症状）、なぜ壊れたのか（原因）、という疑問に答えねばならない

## 6.5 ブラックボックスとホワイトボックス

* ブラックボックスモニタリングは、「現在、システムが正常に動作していない」というような症状を扱う。予測ではなく、実際に起きている問題を表現する
* ホワイトボックスモニタリングは、近々生じることになりそうな問題を検出できる
* ある箇所の症状が、他の個所では原因になることがある

## 6.6 4大シグナル

* **レイテンシ**：リクエストを処理してレスポンスを返すまでにかかる時間
* **トラフィック**：システムに対するリクエストの量。システム固有のメトリックで計測される
