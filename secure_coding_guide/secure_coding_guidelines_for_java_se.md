...
```
システムへの入力は、サービスの要求に比例しない過剰なリソース消費を引き起こさないようにチェックする必要があります。
一般的に影響を受ける資源は、CPUサイクル、メモリ、ディスクスペース、ファイル記述子などである。

まれに、入力が妥当であるかどうかを確認することが現実的でない場合があります。
リソースの確認とデータを処理するロジックを注意深く組み合わせることが必要な場合もあります。
過剰なリソース消費を引き起こす攻撃に加えて、大幅なディスク容量の浪費など、持続的なDoSを引き起こす攻撃にも防御が必要です。
サーバシステムは、外部からの攻撃に対して特に堅牢であるべきである。
```
