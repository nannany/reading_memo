...
- ドメインエラー
  - これはビジネスプロセスの一部として予想されるエラーであり、ドメインの設計に含まれなければなりません。例えば、請求書によって拒否された注文や、無効な製品コードを含む注文などです。ビジネスでは、このようなことに対処するための手順がすでに用意されており、コードにはこれらのプロセスを反映させる必要があります。
- パニック
