...
- リーダーのトランザクション開始時点でオブジェクトを生成したトランザクションがコミット済みであること
- オブジェクトに削除済みのマークがないこと。もしくはマークがある場合、リーダーのトランザクション開始時点でその削除のリクエストを発行したトランザクションがコミットされていないこと
...
スナップショット分離について、Oracleではserializable、postgresやmysqlではリピータブルリードと呼ばれる。

リピータブルリードの正式な定義は研究論文で定義されているらしいが、それを完全に満たすDBMSはほとんどない。
...
```
“通常アトミックな操作は、オブジェクトからの読み取りの際に排他ロックを取り、更新の適用が終わるまで他のトランザクションがそのオブジェクトを読めないようにすることで実装されます。この手法はカーソル固定（cursor stability）と呼ばれることもあります[36, 37]。もう1つの選択肢としては、単純にすべてのアトミックな操作を単一のスレッドで実行するという方法もあります。”
```

#### 7.2.3.2 明示的なロック

select for updateで同時更新を防ぐ
...
レプリケーションについて理解しとかないと厳しい。
5章読まねば。

lww(last writes wins)

```
“残念ながら、レプリケーションを行う多くのデータベースではLWWがデフォルトになっています。”
```
...
分散システムにおいて生じうる問題について論じた。

```
● ネットワーク経由でパケットを送信しようとした場合、そのパケットはロストしたり、どれほど遅延するか分からなかったりする。同様に、レスポンスもロストしたり遅延したりするので、レスポンスを受け取れなかった場合には元々のメッセージが到達したかどうかも分からない。
● ノードのクロックは他のノードと大きくずれているかもしれない(できる限りの努力をしてNTPをセットアップしたとしても)。クロックは急に進んだり戻ったりするかもしれず、たいていはクロックの誤差をうまく計る方法がないので、クロックに依存するのは危険である。
● プロセスは処理中にいつどれほどの長さ一時停止するかもしれず(おそらくはstop-the-worldガベージコレクタのため)、他のノードから落ちていると見なされた後に自身に一時停止があったことを理解しないままに復活するかもしれない。
```

分散システムはシステムの構成要素の一部分が破損していても全体として動き続けられるようにすべき。

フォールトの検出にはタイムアウトが用いられることが多いが、タイムアウトだとノードが悪いのか、ネットワークが悪いのかはわからない。

何かを決めるにあたっては、単一のノードだけでは決定ができないので、ほかのノードとの合意を取る必要がある。

分散システムならではの考えないといけないことがたくさんあるので、耐障害性や低レイテンシを考慮に入れなくても良いならば単一ノードの方がいいかも。
...
複数の異なるデータシステムを結合し、一貫したアプリケーションアーキテクチャにすることにまつわる問題を検証する。

## Ⅲ.1 記録のシステム(Systems of Record) と導出データ

データを保存して処理するシステムは大雑把には2つに分けられる
...
10章はMapReduceのようなバッチ指向のデータフローシステムについて。
11章は10章の概念をデータストリームに適用する？
12章は10,11で見たツールがどのように使えるのかみる。
