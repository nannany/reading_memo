...
* アーキテクチャはユースケースを支える構造

### アーキテクチャの目的

* フレームワーク等の選択肢は後から決められるようにする

### だが、ウェブはどうか？

* ウェブでの提供であれ、それは提供の仕組みの1つにすぎないので、システム構造を支配するものではない
...
* 円の内側の形式から、円の外側で扱うにおいて便利な形式に変換する

### フレームワークとドライバ

* いちばん外側に置かれる。ほとんどコードは書かない

### 4つの円だけ？

* 円が4つだけから構成されるとは限らないが、外が内に依存する構造は変わらない

### 境界線を越える

* コントローラ→ユースケース→プレゼンターという流れで、ユースケースとその他2つの境界線を越えるにあたっても、DIPを使用して、内側に依存性を向ける

### 境界線を越えるデータ

* 境界線を越えてデータを渡すときは、常に内側にとって便利な形式にする

### 典型的なシナリオ

### まとめ

* これらのことをしておけば、外側の円のパーツが廃れても改修が容易


## 23章 プレゼンターとHumble Object

* プレゼンターはHumble Objectパターンの1種

### Humble Objectパターン

* ユニットテストを実行する人が、テストしやすい振る舞いとテストしにくい振る舞いを分離するために生み出されたデザインパターン
* Humble Objectパターンを使用すれば、2種類の振る舞いをPresenterとViewのクラスに分けられる
