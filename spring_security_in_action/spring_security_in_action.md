...
認可サーバーにて、各クライアント毎にどの付与方式に対応しているか記述する。  
1人のクライアントが複数の付与方式に対応することも可能。  
※だけど、１つのクライアントに複数の付与方式を許可するのはできるだけ避けたほうがいい。

クライアント毎に別のパスワードを用意すべき、みたいなところまだ消化不良。

## 13.6 Using the client credentials grant type

クライアントクレデンシャルズフローについて

ex13-ex3で確認

セキュリティ的にゆるいので、スコープの設定の仕方には気をつけること。
...
* ClientRegistrationインターフェイスは、Spring SecurityのOAuth 2クライアント登録を定義します。ClientRegistrationRepositoryインターフェイスは、クライアント登録の管理を担当するオブジェクトを記述します。この2つのコントラクトにより、認可サーバーがクライアント登録を管理する方法をカスタマイズすることができます。
* Spring Securityで実装された認可サーバーでは、クライアント登録によってグラントタイプが決まります。同じ認可サーバーでも、クライアントによってグラントタイプを変えることができます。つまり、複数のグラントタイプを定義するために、認可サーバーに何か特別な実装をする必要はありません。
* 認証コード付与タイプでは、オーソリゼーションサーバーはユーザーにログインの可能性を提供する必要があります。これは、認可コードの流れの中で、ユーザー(リソースの所有者)が認可サーバーで直接認証を行い、クライアントにアクセスを許可するという事実に起因しています。
* ClientRegistrationは、複数のグラントタイプを要求できます。これは、クライアントが、例えば、パスワードと認可コードの両方のグラントタイプを異なる状況で使用できることを意味しています。
* バックエンド間の認証には、クライアントの認証コード付与タイプを使用します。技術的には可能ですが、クライアントがクライアント資格情報グラントタイプと他のグラントタイプを一緒に要求することは稀です。
* リフレッシュ・トークンは、認証コード・グラントタイプやパスワード・グラントタイプと一緒に使用することができます。クライアント登録時にリフレッシュ・トークンを追加することで、認可サーバーにアクセストークン以外にリフレッシュ・トークンも発行するように指示します。クライアントはリフレッシュ・トークンを使って新しいアクセストークンを取得し、ユーザーの認証を再度行う必要がありません。

# 14 OAuth2:Implementing the resource server

リソースサーバーについて見ていく。

重要なのは、リソースサーバーがどのようにトークンを検証するか。  
ここでは３つのオプションを紹介していく。

* リソースサーバーが直接認可サーバーにトークンが正しいか問い合わせる方式 (14.2)
* 認可サーバーとリソースサーバーがDBを共有しており、そこに問い合わせに行く方式
* JWTの署名で検証する方式（リソースサーバーがトークンを受け取ってからのやりとりは発生しない）15章でやる


## 14.1 Implementing a resource server

ssia-ch14-ex1-rsでリソースサーバーを動かす。

`@EnableResourceServer`アノテーションは最近deprecatedとなったらしい。


## 14.2 Checking the token remotely

トークンの検証を認可サーバーにしに行くパターン

メリットはシンプルな点。  
デメリットは認可サーバーに与える負荷が大きく、ネットワークを経由してしまう点。
ネットワーク切れで通信失敗した場合は、トークンが正しくてもアクセスを弾いてしまう。
...
`TokenStore`がコントラクト。JdbcTokenStoreがそれの実装。

ssia-ch14-ex2-as が認可サーバー
ssia-ch14-ex2-rs がリソースサーバー

このblackboardingのデメリットは、アクセス集中したとき、dbアクセスがボトルネックになりやすい点。

## 14.4 A short comparison of approaches

トークンの検証方法の実装方法メリデメの整理
...
* 直接認可サーバーに問い合わせる
  * メリット
    ...
    * 様々なタイプのトークンに適用できる
  * デメリット
    * 認可サーバーとリソースサーバー間で依存関係が生まれる
    * 認可サーバーに与える負荷が大きい
* blackboarding
  * メリット
    * 認可サーバーとリソースサーバーの間の直接の依存関係がなくなる
    * 様々なタイプのトークンに適用できる
    * 認可サーバーのリスタートやダウンの影響を受けにくい
  * デメリット
    * 直接問合せの方より実装は難しい
    * １つコンポーネントが増える
    * dbがボトルネックになりうる
...
* リソースサーバは、ユーザのリソースを管理するSpringコンポーネントです。
* リソースサーバは、認可サーバがクライアントに発行したトークンを検証する方法が必要です。
* リソースサーバのトークンを検証する方法として、認可サーバを直接呼び出す方法があります。この方法は、認可サーバーに大きなストレスを与える可能性があります。私はこの方法を避けるようにしています。
* リソースサーバーがトークンを検証できるようにするために、ブラックボーディング・アーキテクチャを実装することができます。この実装では、認証サーバーとリソースサーバーが同じデータベースにアクセスしてトークンを管理します。
* ブラックボーディングには、リソースサーバーと認可サーバーの間の直接的な依存関係がなくなるという利点があります。しかし、トークンを保持するためのデータベースを追加することになるため、大量のリクエストがあった場合にはボトルネックとなり、システムのパフォーマンスに影響を与える可能性があります。
* トークン管理を実装するには、TokenStore 型のオブジェクトを使用する必要があります。TokenStoreの実装は独自に書くこともできますが、多くの場合、Spring Securityが提供する実装を使用します。
* JdbcTokenStoreはTokenStoreの実装で、アクセストークンとリフレッシュトークンをデータベースに永続化するために使用できます。


# 15 OAuth2: Using JWT and cryptographic signatures

JWTでトークンの妥当性検証を行う術を見ていく。

## 15.1 Using tokens signed with symmetric keys with JWT

対象鍵（共通鍵）を利用して署名するパターンを見ていく。これは、鍵の交換をうまくする場面が限られるのが弱点。
