...
ページテーブルで、仮想アドレスと物理アドレスの対応関係を保持する。
ページテーブルの情報は主記憶に保持される。つまり、メモリ管理を行う情報がメモリ上に載ることになる。
...
というように、2回のメモリアクセスが必要になる。
これは遅くなる要因なので、TLBなる仕組みが用意されている。

## 6.3 **TLBによる緩和**

TLB（Translation Lookaside Buffer）は、ページテーブルの一部をキャッシュすることで、アドレス変換の高速化を図る仕組みである。
TLBはCPUに内蔵されるハードウェアである。
