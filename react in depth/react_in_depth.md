...
Reactの第9章「リモートデータとリアクティブキャッシュ」では、主に**ローカルデータとリモートデータの違い**、**サーバー通信に伴う複雑さ**、そして**リモートデータ管理のためのライブラリの使用**について解説されています。
...
『React in Depth』の第10章は、**Reactコンポーネントの単体テスト**に焦点を当てており、アプリケーションをバグやリグレッションから保護するための実践的な知識と戦略を提供しています。
...
第11章「Reactウェブサイトフレームワーク」では、**サーバーサイドレンダリング（SSR）と静的サイト生成のためのReactの拡張機能**について詳しく解説されています。

この章の主要なポイントは以下の通りです。

*   **ウェブサイトフレームワークとは**
    *   Reactウェブサイトフレームワークの根幹は、Reactをサーバー（Node.jsなど）で実行し、HTMLを生成できる点にあります。
    *   これにより、**フルスタック開発**が可能になり、フロントエンドとバックエンドを単一のアプリケーションで統合できます。
    *   主な利点には、ページ読み込み時間を改善しSEOを向上させる**SSR**、データベースや外部APIからのコンテンツを統合する**動的コンテンツ**、そしてページ作成を容易にする**URLルーティング** が挙げられます。
    *   ただし、SSRには**ハイドレーション**という重要な要件があります。クライアントのReactの出力がサーバー生成のHTMLとバイト単位で一致する必要があり、一致しないとReactがDOMをクリアしてページ全体を再レンダリングするため、パフォーマンスが低下します。**部分的なハイドレーション**は、個々のコンテンツのハイドレーションを可能にする、より新しい概念です。

*   **実装例：Next.jsとRemix**
    *   **Next.js**は、Reactウェブサイトフレームワークのベテランであり、Vercelが開発・サポートしています。この章では、従来の「Pages Router」に焦点を当てています。
    *   **Remix**は、React Routerの作成者であるRyan Florenceによって開発された比較的新しいフレームワークで、JavaScriptがなくても機能するウェブサイトを構築することに重点を置いています。Remix v2では、フラットなルーティングスキームを採用し、ファイル名をピリオドで区切る形式（例: `about.team.jsx`）が導入されました。

*   **環境変数とAPIキー**
    *   両フレームワークとも、APIキーなどの機密情報を`.env`（Remix）や`.env.local`（Next.js）のようなファイルに定義することで、安全に管理できます。

*   **天気アプリの構築**
    *   この章では、Next.jsとRemixを使用して天気アプリを構築し、フレームワークの機能を示します。
    *   **URLの使用**: URL (`/`, `/{country}`, `/{country}/{city}`) を使って異なるページを表示し、Next.jsでは `[param].js`、Remixでは `$param.js` を動的ルートに使用します。
    *   **サーバーでのデータ読み込み**: データパッケージは、Next.jsでは `getServerSideProps`、Remixでは `loader()` のような関数で定義され、サーバー上でデータを読み込み、それをコンポーネントにプロパティとして渡します。
    *   **ローカルデータの保存（クッキー）**: ユーザーの地域設定（例: 温度単位）などのローカル設定は、セッション間で持続し、SSRと連携するために**クッキー**に保存されます。これは、`localStorage`がブラウザのみで機能し、ハイドレーション失敗の原因となるためです。Next.jsでは `cookies-next` ライブラリ、Remixでは組み込みのクッキーユーティリティ (`createCookie`, `parse`, `serialize`) を使用します。Remixのクッキーは暗号化されており、サーバー専用です。
    *   **APIの作成**: 両フレームワークともAPIルート（例: `/api/geocode`）の作成をサポートしています。Next.jsではAPIルートはハンドラー関数を返しますが、Remixではコンポーネントをエクスポートしない通常のルートとして扱われます。

*   **代替フレームワーク**
    *   Next.jsとRemix以外にも、Gatsby（小規模ウェブサイト向け）、Astro（コンテンツ重視のウェブサイト向け）、RedwoodJS（高速プロトタイピング向け）などの代替フレームワークがあります。
