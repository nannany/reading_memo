...
#### 8.2.1. リソースのエンドポイントをどのように守るのか？

Content-Typeを適切に設定することで、XSSを防ぐ。

X-Content-Type-Options: nosniff  
X-XSS-Protection: 1; mode=block  
もうまく使う

そもそもaccess_tokenをリクエストパラメータを介して受け取らないようにする。

流れがまだよくわかってないな。。要復習

#### 8.2.2. インプリシット付与方式へのサポートの追加
...
* 保護対象リソースのレスポンスの信頼できないデータすべてに対してサニタイズを行う
* 対象のエンドポイントには適切なContent-Typeを選択する
* できるだけブラウザが提供する保護の仕組みとセキュリティに関するヘッダを活用する
* もし、保護対象リソースのエンドポイントがインプリシット付与方式のフローをサポートしなければならない場合はCORSを使おう
* 保護対象リソースに対してJSONPはサポートしない
* いつもTLSを使うようにし、その際はHSTSを組み合わせる

## 9. よく狙われる認可サーバの脆弱性

### 9.1 一般的なセキュリティ

一般的に言われるWebサーバ構築の際のアドバイスはすべて認可サーバに取り入れること

### 9.2 セッションの乗っ取り

同じ認可コードで2回アクセスしてきた場合は、2回目ははじかねばならない

#### 307 でリダイレクトさせるとなんかやばいらしい
https://arxiv.org/pdf/1601.01229v2.pdf

307はその前のリクエストと同じメソッドでリダイレクトに行くことになっちゃう。Bodyも同じのが送られるので、悪意のあるクライアントにクレデンシャルがおくられちゃう。

### 9.3 リダイレクトURIの不正操作

リダイレクトURIの検証は、完全一致、サブディレクトリの許可、サブドメインの許可のどれかを使って検証。

安全なのは完全一致のみ。サブディレクトリの許可とかすると、パストラバーサル使って簡単に攻撃者のページにリダイレクトさせることができてしまう。

### 9.4 クライアントのなりすまし

最初の認可リクエストで提示されたredirect_uriとトークンエンドポイントに来た時のredirect_uriが完全一致することをみる

### 9.5 オープンリダイレクトによる脆弱性

### 9.6 まとめ

* 一度使われた認可コードは破棄するようにする
* 認可サーバが採用すべき唯一の安全なredirect_uriの検証方法は完全一致
* OAuth2.0の認可フレームワークの仕様を記述通りに実装してしまうと、オープンリダイレクトの脆弱性を使った攻撃の対象に認可サーバがなってしまう可能性が高くなる。もし、これが適切に監視されている環境下でリダイレクトを行っているのなら問題ないが、何も注意せずにそのまま実装してしまうと何らかの脅威がもたらされる危険性がある。
* エラーを報告する際は、フラグメントやRefererヘッダーを介して情報が漏洩する可能性があることに注意を払う


## 10. よく狙われるOAuthトークンの脆弱性

トークンが盗まれることは、パスワードが盗まれることより被害が少ない。

### 10.1 Bearer トークンとは何か

Bearerトークンのイメージはバスの乗車券。持ち主が誰であれ、乗車券を持っていればバスに乗車することができる。
...
* トークンの偽造(Token Forgery)
  * トークンを修正して、トークンの有効範囲をひろげるなど
* トークンのリプレイ(Token Replay)
  * 有効期限切れのトークンをもってリソースにアクセスする行為
* トークンの流用(Token Redirect)
  * とあるリソースサーバで有効なトークンを、他のリソースサーバで試す行為
...
* トークンの持つスコープを必要最低限のものにする
* インジェクションなどの被害をかんがみて、DBなどにアクセストークンをおかない

#### 10.3．2 認可サーバでの保護

* トークンの有効期限をAPIの使用に必要な平均時間より長くしない
* 認可サーバ内で監視、ログ出力をして、疑わしいふるまいを監視する

#### 10.3.3 保護対象リソースでの保護

* ログにトークンの値を出さない
* クライアントにて、スコープを適切に設定できるように、リソースの分割を適切に行う
* キャッシュに対する長所短所をきちんと考える(11章)

###　10.4 認可コード

公開クライアントに対してはPKCEを使うべし。
...
* アクセストークンの送信はTLSのような安全なトランスポート層の仕組みを使って保護されなければならない
* クライアントは必要最低限の情報のみを問い合わせるべきである(保守的にスコープを使うようにする)
* 認可サーバはアクセストークンを平文でなくハッシュ値にして格納すべき
* 認可サーバはアクセストークンの有効期限を短くし、例えば、一つのアクセストークンが漏洩したとしても、そのことに対するリスクが最小になるように対策すべき
* リソースサーバはアクセストークンを一時的なメモリーに保持すべき
* 認可コードの安全性を高めるためにPKCEが使われることがある

#　第4部 さらなるOAuthの活用

OAuthの中核から離れて、その補助的な構成要素についてみていく
